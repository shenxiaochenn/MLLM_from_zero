...
if __name__ == '__main__':
    config = VLMConfig()# 这里我懒了。。之间填在上面了。。不要学我! shenxiaochen
    model = VLM(config).cuda()# 这里我们只训练那个对齐层，我们第一步训练就训练这个！
    print(model)
    print(f'模型参数量为：{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    images_path = '/mnt/data/shenxiaochen_data/multimodel/images_'
    data_path = '/mnt/data/shenxiaochen_data/multimodel/chat-translated.json'
    tokenizer = AutoTokenizer.from_pretrained(config.llm_model_path)
    processor = AutoProcessor.from_pretrained(config.vision_model_path)
    output_dir = 'save/pretrain' 
    args = TrainingArguments( #这个可以加eval。。加几个参数。。去搜搜。。
